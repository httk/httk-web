...
`httk-serve` should be run as an **ASGI app** (for example with `uvicorn`) behind Nginx reverse proxy.
...
This configuration redirects HTTP to HTTPS and proxies requests to the `httk-serve` ASGI upstream.
