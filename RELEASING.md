...
Node/npm are also required for the JavaScript checks. Browser acceptance
remains a separate `make test-browser` gate.
...
/tmp/httk-serve-test/bin/python -m pip install \
  ...
  --extra-index-url https://pypi.org/simple/ httk-serve==2.1.0
/tmp/httk-serve-test/bin/python -c "import httk.serve.web, httk.serve.optimade"
```

Replace `2.1.0` with the version being tested. `httk-serve` has runtime
